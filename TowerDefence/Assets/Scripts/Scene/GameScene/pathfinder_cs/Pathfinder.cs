...
{  
    ...
    public void CreateNode( int _x, int _z, float _width, Vector3 _offset)
    {
        nodeGroup = new NodeGroup(_x, _z, _width, _offset );
    }
    public List<Vector3> FindPath(Vector3Int _startPos, Vector3Int _endPos)
    {
        return FindPath(_startPos.x,_startPos.y,_endPos.x,_endPos.y);
    }
    public List<Vector3> FindPath( Vector3 _startPos, Vector3 _endPos )
    {
        ...
        return FindPath(nodeGroup.GetNodePosition(_startPos), nodeGroup.GetNodePosition(_endPos));
    }
    ...
    private List<Node> FindPathActual(Node start, Node target)
    {
        if (null == start)
        {
            Debug.LogError("FindPathActual()[null == start]");
            return null;
        }
        if (null == target)
        {
            Debug.LogError("FindPathActual()[null == target]");
            return null;
        }
    ...
    private Node GetNeighbourNode(Node adjPos, bool searchTopDown, Node currentNodePos)
    {
        ...
        Node node = GetNode(adjPos.x, adjPos.z);
        ...
        else if (searchTopDown)//and we want to have 3d A* 
        {
            ...
            //adjPos.y -= 1;
            Node bottomBlock = GetNode(adjPos.x, adjPos.z);
            ...
            else
            {
                ...
                //adjPos.y += 2;
                Node topBlock = GetNode(adjPos.x, adjPos.z);
    ...
    public bool IsEnableTower( Vector3 _pos )
    {
        Node _node = nodeGroup.GetNodePosition(_pos);
        if (null == _node)
            return false;

        return _node.isBuildable;
    }
    public bool IsEnableTower(Vector3Int _pos)
    {
        Node _node = nodeGroup.GetNode(_pos.x,_pos.y);
        if (null == _node)
            return false;
    ...
    public bool isCanTower(Vector3Int towerPosition,List<Node> turningNode)
    {
        ...
        for (int x = towerPosition.x - 1; x < towerPosition.x + 1; x++)
        {
            ...
            {
                Node node = nodeGroup.GetNode(x, y);                
                towerNodes.Add(node);
            }
        }

        if (towerNodes == null || towerNodes.Count <= 0)
            return false;


        TowerNodesisWalkalble(towerNodes, false);

        bool returnVal = false;
        Vector3Int startPos = GridTile.Ins.GetStartPosition();
        for(int i=0; i<turningNode.Count-1;i++)
        {
            List<Vector3> path = FindPath(turningNode[i], turningNode[i+1]);
        ...
        TowerNodesisWalkalble(towerNodes, true);
    ...
    private void TowerNodesisWalkalble(List<Node> towerNodes,bool isWalkable)
    {
        for (int i = 0; i < towerNodes.Count; i++)
        {
            {
                towerNodes[i].isWalkable = isWalkable;
            }
        }
    }
    ...
    public void DrawGizmos()
    {
        ...
        Gizmos.DrawSphere(m_startPos, 0.1f);
        Gizmos.color = Color.blue;
        Gizmos.DrawSphere(m_endPos, 0.1f);

        if (null != nodeGroup)
            nodeGroup.DrawGizmos();
    }
}
